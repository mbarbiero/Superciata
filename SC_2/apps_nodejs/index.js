...
const { executa_sql, executa_arquivo_sql } = require('./sc2_dbExecutor');
...
app.post('/sc2_/executa_sql', async (req, res) => {
  ...
  try {
    const { tipo, sql, arquivo, parametros } = req.body;

    let resultado;

    if (tipo === 'arquivo') {
      // Passa o nome do arquivo e o objeto de parâmetros
      resultado = await executa_arquivo_sql(arquivo, parametros, 'Painel_Template');
    } else {
      // ERRO ANTERIOR: Você estava passando 'req.body' (objeto)
      // CORREÇÃO: Passar apenas 'sql' (string)
      resultado = await executa_sql(sql, 'Painel_Direto');
    }

    res.json(resultado);
  } catch (err) {
    console.error("Erro na rota SQL:", err.message);
    res.status(500).json({ sucesso: false, erro: err.message });
  }
});
...
app.post("/superciata/carrega_arqZip", upload.single("arqZip"), async (req, res) => {
  try {
    ...
    await fs.createReadStream(zipPath)
      .pipe(unzipper.Extract({ path: CsvDir })).promise();

    arquivos.deletaArquivo(zipPath);

    const { tipo, nome_csv, municipio_cod } = req.body;
    console.log(tipo);
    const parametros = { COD_MUNICIPIO: municipio_cod, ARQUIVO_CSV: path.join(config.paths.csvDir, nome_csv) };
    if (tipo.toLowerCase() === 'cnefe') {
      resultado = await executa_arquivo_sql('CN_PONTOS_Preenche.sql', parametros, 'Painel_Template');
    } else if (tipo.toLowerCase() === 'ciata') {
      resultado = await executa_arquivo_sql('CI_LOTES_Preenche.sql', parametros, 'Painel_Template');
    } else {
      throw new Error('Não é um arquivo do tipo cnefe ou ciata.');
    }
    res.json({
      sucesso: true,
